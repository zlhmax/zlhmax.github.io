  ...
  return (
    <a href={`/blog/${item.id}`} className="group flex flex-col gap-2 p-3 border border-border/50 hover:border-border active:border-border hover:bg-muted animation hover:scale-102 active:scale-100 select-none">
      <div className="flex items-center justify-between">
        <span className="text-[11px] text-muted-foreground group-hover:bg-background group-hover:px-2 animation px-1">{item.data.category}</span>
        <div className="flex items-center gap-2 shrink-0 pt-1">
          <span className="text-[11px] text-muted-foreground/80 group-hover:translate-x-0 translate-x-6 animation">{formattedDate}</span>
      ...
      <div className="flex flex-col items-start justify-start px-1">
        ...
        <p className="mt-0.5 text-base text-muted-foreground">{item.data.description}</p>
      </div>
    </a>
  );
}
