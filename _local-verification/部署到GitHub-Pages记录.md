...
## 九十六、Blog 正文「标题字号」小一号 · 2026-09-26

**需求**：Eddy「段落标题的字号能否减小一号？」

**① 换算**：按本项目惯例「号」= **2px/号** → 各级 −2px，且**保持 28 > 22 > 18 > 正文 16 的层级**（若照搬模板步长 30→24 会让 h4 掉到 16 = 正文字号，层级崩塌，故不用）。

| 层级 | 改前 | 改后 | Tailwind |
|---|---|---|---|
| `h2` | `text-3xl` 30px | **28px** | `text-[28px]/[1.2]` |
| `h3` | `text-2xl` 24px | **22px** | `text-[22px]/[1.3333]` |
| `h4` | `text-xl` 20px | **18px** | `text-[18px]/[1.4]` |

**② ⚠️ 本轮关键坑：Tailwind 的「任意字号」不带行高，必须显式带**

- 改前实测行高比：**h2 ×1.2（36px）· h3 ×1.3333（32px）· h4 ×1.4** —— 来自 `text-3xl/2xl/xl` 自带的 `--text-*-line-height`。
- 若只写 `text-[28px]`：该工具类**只输出 `font-size`**，行高会**继承 `.prose` 的 `leading-relaxed`（1.625）** → 28px × 1.625 = **45.5px** ✗✗
  **字号变小了、行距反而变大**，视觉上标题更松——与诉求相反。
- **正解 = 用 v4 的「字号/行高」简写语法**：`text-[28px]/[1.2]` → 编译出 `font-size:28px; line-height:1.2` ✓（实测计算值 33.6px ✓）。
- 验证：改完**必须同时量 `fontSize` 与 `lineHeight`**，并算比值是否与改前一致（1.2 / 1.3333 / 1.4）。

**③ 验收（本地 + 线上三篇逐一）**

| 项 | 改前（线上）| 改后（线上）|
|---|---|---|
| h2（markdown-reference ×10、autocad-flashquite ×2）| 30px / lh 36px / 盒高 49px | **28px / lh 33.6px(×1.2) / 盒高 46.59px** ✓ |
| h3（markdown-learn ×3、autocad-flashquite ×1）| 24px / lh 32px / 盒高 34.33…… | **22px / lh 29.3326px(×1.3333)** ✓ |
| h4 | 无用例（配置 20→18px）| — |
| 正文 `p` | 16px / lh 26px | **16px / 26px 未动** ✓ |
| `margin-top` / `margin-bottom` | 42/20（h2）· 20/16（h3）| **完全未动** ✓ |
| 线上 CSS sha256 | — | 3 个文件全一致 ✓（`Head.DlZycvXz.css` 等），部署约 66 秒 |

- **目视前后对照**（`Desktop/标题字号_前后对照_20260926.png`）：28px 与正文 16px 层级仍清晰（1.75×），标题仍醒目（600 字重 + 下分隔线 + 42px 上间距）。
- 本次**只动字号**：字重 600 / 下边框 / `pb-3`·`pb-1` / 上间距 42px（上轮刚减半）/ 下间距 mb —— 全部保持。

**④ 提交**：`0e686b7 feat(prose): reduce heading font sizes by one step`（1 文件 7 增 4 删）

**⑤ 正文排版当前完整档位**：
h2 **28px**/行高 1.2/mt 42px/mb 20px/pb 12px/下边框 · h3 **22px**/1.3333/mt 20px/mb 16px/pb 4px/下边框 · h4 **18px**/1.4/mt 16px/mb 12px · 正文 16px/1.625 · 字重均 600（标题）。
备选档：再小一号 = 26/20/17 或 26/20/16（h4 会与正文同号，不推荐）。


---

## 九十七、Blog 正文标题字号再小一号 → **26 / 20 / 18px** · 2026-09-26

**需求**：Eddy「再小一号 → 26/20/18」（用户直接指定了三级的具体值，含「h4 停在 18」的判断）

**① 改动（1 文件 3 行）** —— 只改字号，行高比例仍用 `text-[Npx]/[ratio]` 简写保住

| 层级 | 上一档 | **本档** | 行高比 | 编译后实测 |
|---|---|---|---|---|
| `h2` | `text-[28px]/[1.2]` | **`text-[26px]/[1.2]`** | 1.2 | 26px / 31.2px ✓ |
| `h3` | `text-[22px]/[1.3333]` | **`text-[20px]/[1.3333]`** | 1.3333 | 20px / 26.666px ✓ |
| `h4` | `text-[18px]/[1.4]` | **不变（18px）** | 1.4 | 18px（无文章用）|

层级：**26 > 20 > 18 > 正文 16** ✓ 仍成立（若 h4 再降一级到 16 就会与正文同号，故停住）。

**② 验收（本地 + 线上三篇逐一）**

| 项 | 线上实测 |
|---|---|
| h2（markdown-reference ×10、autocad-flashquite ×2）| font-size **26px** · line-height **31.2px(×1.2)** · 标题盒高 **44.19px** · mt 42px / mb 20px |
| h3（markdown-learn ×3、autocad-flashquite ×1）| font-size **20px** · line-height **26.666px(×1.3333)** · 盒高 **31.66px** · mt 20px / mb 16px |
| 正文 `p` | 16px / 26px **未动** ✓ |
| 字重/边框/padding/mt/mb | **全部未动** ✓ |
| 线上 CSS sha256 | 3 个文件全一致 ✓（`Head.BQGqWVyF.css` 等），部署约 66 秒 |

各档标题盒高演进（h2）：49px（30px 时）→ 46.59px（28px）→ **44.19px（26px）**；h3：34.33 → **31.66px**。
视觉：26px 与正文 16px 对比仍分明（1.625×），标题下灰色分隔线比例协调，整体更收敛 ✓（截图 `Desktop/标题字号_26px_20260926.png`）

**③ 提交**：`7b6732e feat(prose): reduce heading font sizes one more step`（1 文件 3 增 3 删）
